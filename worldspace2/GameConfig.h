...
static struct {
	...
	/**
	 *  @brief	This is the value for (lambda) in an exponential distribution function.
	 *\n		It is used to select actor templates less often when placed at high (relative) indexes in the ActorTemplate vectors above.
	 *\n		The lower the value, the closer to a straight line. The higher the value, the more curved.
	 *\n		See this image for a graphic: ( https://en.wikipedia.org/wiki/Exponential_distribution#/media/File:Exponential_cdf.svg ).
	 */
	double npcDistribRate{ 1.0 }; 
	/**
	 * @brief	This value is used in the calculation of NPC aggressiveness.
	 *\n		Its scale goes from (0.0 - 2.0), which is scaled to (-1.0 - 1.0)
	 *\n		
	 */
	float npcFearAggressionScale{ 0.4f };
} GameConfig;

inline float getNPCFearAggressionScale()
{
	return 1.0f - GameConfig.npcFearAggressionScale;
}
