...
class Gamespace final {
	...
	[[nodiscard]] Coord findValidSpawn(bool isPlayer = false, bool checkForItems = true)
	{
		...
		const long max_checks{ (_world._max._x - 2) * (_world._max._y - 2) };
		// loop
		for (long i{ 0l }; i < max_checks; i++) {
			Coord pos{ 0, 0 };
			for (auto findPos{ pos }; !_world.get(findPos)->_canSpawn; pos = findPos)
				findPos = { _rng.get(1l, _world._max._x - 2), _rng.get(1l, _world._max._y - 2) };
			// Check if this pos is valid
			if (!checkForItems ? true : getItemAt(pos) == nullptr && isPlayer ? true : getActorAt(pos) == nullptr && getDist(_player.pos(), pos) >= static_cast<long>(_ruleset._enemy_aggro_distance + _player.getVis() * 2))
				return pos;
		}
	...
	template<typename Actor> [[nodiscard]] std::vector<Actor> generate_NPCs(int count, std::vector<ActorTemplate>& templates)
	{
		...
		for (auto i{ 0 }; i < count; i++) {
			...
			for (size_t it{ 0 }; it < templates.size(); ++it) {
				if (_rng.get(0.0f, 100.0f) < templates.at(it)._chance) {
					sel = it;
					break;
				}
			}
	...
	void trap(ActorBase* actor, bool didMove)
	{
		...
		if (_world.get(actor->pos())->_isTrap && didMove && !(actor->faction() == FACTION::PLAYER && _ruleset._player_godmode)) {
			...
			// Check if actor died
			if (actor->isDead())
				actor->killedBy(_ruleset._killed_by_trap.at(_rng.get(_ruleset._killed_by_trap.size() - 1u, 0u)));
		}
	}
	/**
	 * move(ActorBase*, char)
	...
	int attack(ActorBase* attacker, ActorBase* target)
	{
		...
		if (attacker->getStamina() >= _ruleset._attack_cost_stamina) {
			...
			if (target->getStamina() >= _ruleset._attack_cost_stamina && _rng.get(0.0f, _ruleset._attack_block_chance) < 1.0f) {
	...
	bool actionNPC(NPC* npc)
	{
		...
		if (_game_state._final_challenge.load() && (npc->faction() == FACTION::ENEMY || npc->faction() == FACTION::NEUTRAL && _ruleset._challenge_neutral_is_hostile)) {
			...
			// npc is aggravated
				...
				//if ( _rng.get(0.0f, 100.0f) < _ruleset._npc_move_chance_aggro )	{
			...
			// npc is idle, check nearby
				...
				if (nearest != nullptr && npc->canSeeHostile(&*nearest)) {
					...
					if (npc->setTargetMaxAggro(&*nearest))
						rc = moveNPC(&*npc);
					//else if ( _rng.get(0.0f, _ruleset._npc_move_chance) < 1.0f )
					//	rc = move(&*npc, getRandomDir());
				}
				else if (_rng.get(0.0f, 100.0f) < _ruleset._npc_move_chance)
					rc = move(&*npc, getRandomDir());
				// else do nothing
			}
		}
...
public:
	...
	{
		return !_FLARE_QUEUE.empty() ? _FLARE_QUEUE.at(0) : nullptr;
	}
	/**
	 * resetFlare()
