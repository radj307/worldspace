...
#include <sys.h>	// for system commands
		...
		inline void print_game_over(memory& mem, const std::optional<const COORD>& textPos = std::nullopt)
		{
			...
			case PLAYER_LOSE_CODE: // player lost the game
				[&pos](const std::optional<std::string>& killer) {
					...
					std::cout << Color::f_red << loseText << Color::reset;
					if ( killer.has_value() ) {
			...
			case PLAYER_QUIT_CODE: // player exited the game
				std::cout << Color::f_cyan << "Game Over." << Color::reset;
				break;
			case GAME_EXCEPTION_CODE:
			default: // undefined
				std::cout << Color::f_red << "The game exited with an undefined error." << Color::reset;
				break;
			}
