...
public:
	...
	*///		return actor != nullptr && CAN_LEVEL_UP(actor->getLevel(), actor->getKills());
	...
	static void setINITemplate(ActorTemplate& target, file::INI& cfg, const std::string& section)
	{
		if (cfg.check(section)) {
			target = ActorTemplate{
				...
				ActorStats(cfg.getvs_cast<int>(section, "level", str::stoi).value_or(target._stats.getLevel()),
				cfg.getvs_cast<int>(section, "health", str::stoi).value_or(target._stats.getHealth()),
				cfg.getvs_cast<int>(section, "stamina", str::stoi).value_or(target._stats.getStamina()),
	...
	explicit GameRules(file::INI& cfg) :
		_walls_always_visible(cfg.getvs_cast<bool>("world", "showAllWalls", str::stob).value_or(_walls_always_visible)),
		_override_known_tiles(cfg.getvs_cast<bool>("world", "showAllTiles", str::stob).value_or(_override_known_tiles)),
		_dark_mode(cfg.getvs_cast<bool>("world", "fogOfWar", str::stob).value_or(_dark_mode)),
		_cellSize(cfg.getvs_cast<long>("world", "sizeH", str::stol).value_or(_cellSize._x), cfg.getvs_cast<int>("world", "sizeV", str::stol).value_or(_cellSize._y)),
		_trap_dmg(cfg.getvs_cast<int>("world", "trapDamage", str::stoi).value_or(_trap_dmg)),
		_trap_percentage(cfg.getvs_cast<bool>("world", "trapDamageIsPercentage", str::stob).value_or(_trap_percentage)),
		_attack_cost_stamina(cfg.getvs_cast<int>("actors", "attackCostStamina", str::stoi).value_or(_attack_cost_stamina)),
		_attack_block_chance(cfg.getvs_cast<float>("actors", "attackBlockChance", str::stof).value_or(_attack_block_chance)),
		_attack_miss_chance_full(cfg.getvs_cast<float>("actors", "attackMissChanceFull", str::stof).value_or(_attack_miss_chance_full)),
		_attack_miss_chance_drained(cfg.getvs_cast<float>("actors", "attackMissChanceDrained", str::stof).value_or(_attack_miss_chance_drained)),
		_player_godmode(cfg.getvs_cast<bool>("player", "godmode", str::stob).value_or(_player_godmode)),
		_npc_move_chance(cfg.getvs_cast<float>("actors", "npcMoveChance", str::stof).value_or(_npc_move_chance)),
		_npc_move_chance_aggro(cfg.getvs_cast<float>("actors", "npcMoveChanceAggro", str::stof).value_or(_npc_move_chance_aggro)),
		_npc_vis_mod_aggro(cfg.getvs_cast<int>("actors", "npcVisModAggro", str::stoi).value_or(_npc_vis_mod_aggro)),
		_level_stat_mult(cfg.getvs_cast<bool>("actors", "multStatsByLevel", str::stob).value_or("false")),
		_enemy_count(cfg.getvs_cast<int>("enemy", "count", str::stoi).value_or(_enemy_count)),
		_enemy_aggro_distance(cfg.getvs_cast<int>("enemy", "aggroDistance", str::stoi).value_or(_enemy_aggro_distance)),
		_neutral_count(cfg.getvs_cast<int>("neutral", "count", str::stoi).value_or(_neutral_count)),
		_regen_timer(cfg.getvs_cast<int>("actors", "regen_time", str::stoi).value_or(2)),
		_regen_health(cfg.getvs_cast<int>("actors", "regen_health", str::stoi).value_or(_regen_health)),
		_regen_stamina(cfg.getvs_cast<int>("actors", "regen_stamina", str::stoi).value_or(_regen_stamina)),
		...
		_level_up_restore_percent(cfg.getvs_cast<int>("actors", "levelRestorePercent", str::stoi).value_or(_level_up_restore_percent)),
		_enable_boss(cfg.getvs_cast<bool>("enemy", "enable_boss", str::stob).value_or(_enable_boss)),
		_boss_spawns_after_final(cfg.getvs_cast<bool>("enemy", "bossDelayedSpawn", str::stob).value_or(_boss_spawns_after_final))
	{
		assert(!cfg.empty());
		...
		if (cfg.check_any("player", "health"))	///< @brief Check health
			_player_template._stats.setMaxHealth(cfg.getvs_cast<int>("player", "health", str::stoi).value_or(_player_template._stats.getMaxHealth()));
		if (cfg.check_any("player", "stamina"))	///< @brief Check stamina
			_player_template._stats.setMaxStamina(cfg.getvs_cast<int>("player", "stamina", str::stoi).value_or(_player_template._stats.getMaxStamina()));
		if (cfg.check_any("player", "damage"))	///< @brief Check damage
			_player_template._stats.setMaxDamage(cfg.getvs_cast<int>("player", "damage", str::stoi).value_or(_player_template._stats.getMaxDamage()));
